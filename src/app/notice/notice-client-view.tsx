...
export default function NoticeClientView({
  ...
  const handleOpenNotice = (notice: NoticeItem) => {
    ...
    if (typeof window !== 'undefined' && notice._id) {
      window.history.replaceState(null, '', `/notice?id=${notice._id}`);
    }
  };
  ...
  const handleCloseNotice = () => {
    ...
    if (typeof window !== 'undefined') {
      window.history.replaceState(null, '', '/notice');
    }
  };

  const handleShareNotice = (notice: NoticeItem) => {
    if (typeof window === 'undefined' || !notice._id) return;
    const url = `${window.location.origin}/notice?id=${notice._id}`;
    if (navigator.share) {
      navigator
        .share({
          title: notice.title,
  ...
  return (
    <PageLayout
      ...
      subtitle="Stay informed with official Annual General Meetings (AGM), committee updates, financial statements, and public announcements from Madhyanchal Sarbajanin."
